...
void ListaDoblementeEnlazada::eventos(int dato, string cientifico, Evento evento) {
    ...
    string cientificoB = obtenerCientifico();
    if (eventoCreado == 1 && esPrimo(dato) && realizadoB == false) {
        setDatoEvento2(dato);
        if (datoEvento1 != datoEvento2) {
            eventoNuevo = evento.EventoTipoB();
            cout << datoEvento2 << endl;
            cout << "[" << datoEvento1 << "|" << "E" << "|" << "A" << "]" << "->" <<"[" << datoEvento2 << "|" << cientificoB << "|" << "B" << "]" << endl;
            realizadoB = true;
            eventoCreado = 2;
        }
    }
        ...
        cout << datoEvento3 << endl;
            if (esPrimo(datoEvento3)) {
                ...
                cout << "[" << datoEvento1 << "|" << "E" << "|" << "A" << "]" << "->" << "[" << datoEvento2 << "|"
                     << cientificoB << "|" << "B" << "]" << "->" << "[" << datoEvento3 << "|" << cientificoC << "|" << "C" << "]" << endl;
                realizadoA = false;
                realizadoB = false;
                eventoCreado = 0;
            } else {
